<?php
    ...
    if(!empty($_POST)) {
        ...
        $passWord = $_POST['pswd'];
        $tsql = $conn->prepare("SELECT * FROM logins WHERE USERNAME = '".$userName."' AND PASSWORD = '".$passWord."' ");
        $tsql->execute();
        $count = 0;
...
<body class="bg-dark">
    <div class="container mt-5 border rounded  bg-warning">
        <form action="<?php echo $_SERVER["PHP_SELF"] ?>" method="POST" class="p-3">
            ...
            <input type="submit" class="btn btn-primary text-center" value="Login" name="login">
